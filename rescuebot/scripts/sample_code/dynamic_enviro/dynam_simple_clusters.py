	...
	def scan_received(self, msg):
		...
		for i in range(image.shape[0]):
			for j in range(image.shape[1]):
				...
				#reference the odds for coloration
				if self.odds_ratios[i,j] < 1/50.0:
		...
		#if dynamic obstacles have been recorded, lets find the center of them, and track a path 		
		sumx = 0
		sumy = 0
		...
		#plot robot and cluster paths over time
